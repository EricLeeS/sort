...
public class ResponseTemplate extends Generator {
	...
	public ResponseTemplate(String path, String packageName, String ClassName){
		this.outPath = path;
	...
	public Map generateData() {
		...
		response.setSuperClass("BaseResponse");
		//Annotation
		...
		for(PropertyModel prop : this.propList){
			...
			if(prop.isContainer())
				element.addValue("type = "+ prop.getClassName() +".class");
			else
				element.addValue("type = "+ prop.getType() +".class");
